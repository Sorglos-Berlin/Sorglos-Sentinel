...
def export_statistics_html(statistics: dict, path: Path) -> Path:
    ...
    rows = "".join(
        ...
        f"<td>{scan.get('findings', 0)}</td><td>{scan.get('overall_risk', 0)}/100</td>"
        f"<td>{scan.get('health_score', 0)}/100</td></tr>" for scan in scans
...
<meta name="viewport" content="width=device-width"><title>Network Sentinel – Statistikbericht</title>
...
<h1>Statistikbericht</h1><p class="muted">Netzwerk {html.escape(str(statistics.get('selected_subnet', '—')))} · Lokal erzeugt · {html.escape(str(latest.get('timestamp', 'Noch keine Daten')))}</p>
<div class="stats"><div class="card"><strong>{statistics.get('total_scans', 0)}</strong><span>Gespeicherte Scans</span></div>
<div class="card"><strong>{latest.get('health_score', 0)}</strong><span>Aktueller Zustand</span></div>
<div class="card"><strong>{statistics.get('best_health', 0)}</strong><span>Bester Zustand</span></div>
<div class="card"><strong>{statistics.get('worst_risk', 0)}</strong><span>Höchstes Risiko</span></div></div>
...
def console_report(result: ScanResult) -> None:
    ...
    for device in result.devices:
        ...
        print(f"{device.ip:<16} {device.hostname[:27]:<28} "
              f"{ports[:23]:<24} {device.risk_score:>3} {device.risk_category}")
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for device in result.devices:
            writer.writerow([
                ...
                ",".join(map(str, device.open_ports)), device.risk_score,
...
def export_html(result: ScanResult, path: Path) -> Path:
    ...
    summary_cards = "".join([
        f'<div class="stat"><b>{summary.get("health_score", 100)}</b>'
        f'<span>Netzwerkzustand</span></div>',
        f'<div class="stat"><b>{summary.get("highest_risk", 0)}</b>'
        f'<span>Kritischstes Gerät</span></div>',
        f'<div class="stat"><b>{summary.get("coverage_percent", 0)}%</b>'
    ...
    for index, device in enumerate(result.devices):
        ...
        rows.append(
            ...
            f"<td>{html.escape(device.ip)}</td><td>{html.escape(device.hostname or '—')}</td>"
            f"<td>{html.escape(device.mac or '—')}</td><td>{html.escape(device.vendor or '—')}</td>"
            f"<td>{html.escape(', '.join(map(str, device.open_ports)) or '—')}</td>"
            f'<td><span class="badge" style="background:{color}">{device.risk_score} '
        ...
        finding_rows = "".join(
            ...
            f"<p><small>{html.escape(item.finding_type)} · "
            f"Konfidenz {html.escape(item.confidence)}</small><br>"
        ...
        ) or "<li>Keine relevanten Befunde.</li>"
        compliance = " · ".join(
        ...
        details.append(
            ...
            f"<p>{html.escape(device.ip)} · Risiko {device.risk_score}/100 · {compliance}</p>"
            f"<p><strong>Audit-Abdeckung:</strong> "
            f"{html.escape(', '.join(f'{key}: {value}' for key, value in device.audit_coverage.items()))}</p>"
...
<p><strong>Gesamtrisiko:</strong> {summary.get("overall_risk", 0)}/100 ·
<strong>Kritischstes Gerät:</strong> {html.escape(str(summary.get("highest_risk_device", "—")))} ·
